...
public class ObjectivesController(ISender _sender) : ControllerBase
{
    ...
    [HttpGet("{objectiveId}")]
    public async Task<IActionResult> GetById(int objectiveId, int questId)
    {
        var query = new GetObjectiveQuery(objectiveId, questId);
    ...
    public async Task<IActionResult> Complete(int objectiveId, int questId, string answer)
    {
        ...
        Result<CompleteObjectiveResult> result = await _sender.Send(command, HttpContext.RequestAborted);
